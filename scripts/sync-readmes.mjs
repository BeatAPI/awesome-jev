...
const configs = [
  {
    file: 'README.md', index: 0, allHeading: `All ${projects.length} projects`, starWord: 'stars',
  ...
  {
    file: 'README.zh-CN.md', index: 1, allHeading: `全部 ${projects.length} 个项目`, starWord: 'Star',
    strong: '只整理 50+ Star、经过源码核对的 JEV 相关项目、集成、工具、开放模型、实验与生态资源。',
    policy: '收录门槛为 GitHub 50 Star 及以上。看看 JEV 如何完成选择、评分、路由与过滤，同时由应用代码掌控执行。',
  },
  {
    file: 'README.ja.md', index: 2, allHeading: `全 ${projects.length} プロジェクト`, starWord: 'Star',
...
for (const config of configs) {
  ...
  const categoryMarkup = `<p align="center">\n${categories.map(([key, labels], i) => `  <strong>${labels[config.index]} · ${categoryCounts.get(key) ?? 0}</strong>${i === categories.length - 1 ? '' : i === 4 ? '<br>' : ' ·'}`).join('\n')}\n</p>`;
  text = text.replace(/<h2 align="center">(?:Browse by category|按分类浏览|カテゴリから探す)<\/h2>\s*<p align="center">[\s\S]*?<\/p>/, (block) => `${block.match(/^<h2[^\n]+/)[0]}\n\n${categoryMarkup}`);

  const list = projects.map((project) => `- [${project.name}](${project.repoUrl}) · ${project.starsAtCapture.toLocaleString('en-US')} ${config.starWord}`).join('\n');
